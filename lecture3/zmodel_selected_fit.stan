functions {
  ...
  /* This is the fraction of possible data generated by a system at
     redshift `ztrue` that is detectable (zobs < 6). */
  real P_bar_det(real ztrue, real zth) {
    return 0.5*erfc((5.0*(log(ztrue) - log(zth))/sqrt(2.0)));
  }
  ...
  /* This is the integrand that will end up giving us N:
     ...
     dI/dz = Pbar(z)*dN/dz(z)

     (Compared to `zmodel_errors.stan`, there is a factor of Pbar here
     because we need to take account of the selection function in
     computing the expected number of detections.)

     Here the "state" of the ODE is one-dimensional (it is I(z)), the
     parameters are {N0, alpha, beta, zth}, and there is no real
     "data", x_r, or integer "data", x_i.  (Stan distinguishes between
     data and parameters here because it will compute the gradient of
     the integral with respect to the parameters but not the data; so
...
*/
  real[] N_integrand(real z, real[] state, real[] params, real[] x_r, int[] x_i) {
    ...
    dstatedz[1] = P_bar_det(z, zth)*dNdz(z, N0, alpha, beta);
...
parameters {
  ...
  /* We now assume that we don't know the "threshold redshift" that we
     use for down selection.  However, if the threshold is smaller
     than any observed redshift, the P_det term in the likelihood
     product will vanish; since Stan is not happy with likelihoods
     that suddenly become zero, we will instead implement the
     constraint that z_th > z_obs for all z_obs. */
  real<lower=max(zobs), upper=10> zth;
...
model {
  ...
  for (i in 1:nobs) {
    /* Here is the product of likelihoods for zobs given ztrue,
       sigma. The `T[,5]` part is due to the selection effects: we
       have zobs < 6 for the system to be observable, so we need to
       truncate the likelihood distribution to z \in [0, 6]. 

       Also, Stan can no longer "vectorize" statements with
       truncation, so we had to move it inside the loop so it gets a
       bit slower.*/
