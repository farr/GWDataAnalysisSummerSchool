...
data {
  ...
  real sigmaobs[nobs]; /* The associated uncertainty. */
...
parameters {
  ...
  /* We have a parameter for the true redshift for each
     observation, and we have limited them to 0 < z < 10. */
...
model {
  ...
  /* Here is the product of likelihoods for zobs given ztrue, sigma. */
  zobs ~ lognormal(log(ztrue), sigmaobs);

  /* For each observation, we accumulate a factor of dNdz(ztrue). */
  for (i in 1:nobs) {
